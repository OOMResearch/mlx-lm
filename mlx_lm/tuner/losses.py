...
def _make_js_forward_kernel():
...

    {
        ...
        if (thread_index_in_threadgroup == 0) {
            ...
            lse_q_out[0] = static_cast<T>(lse_q);
            lse_p_out[0] = static_cast<T>(lse_p);
        }
    }
    """
    ...
    return mx.fast.metal_kernel(
        name="js_forward",
        input_names=["logits_q", "logits_p"],
        output_names=["out", "out_kl_q", "lse_q_out", "lse_p_out"],
        source=source,
        ensure_row_contiguous=True,
    )
...
def _make_js_backward_kernel():
    ...
    float lse_q = lse_q_in[0];
    float lse_p = lse_p_in[0];
    ...
    return mx.fast.metal_kernel(
        name="js_backward",
        input_names=["logits_q", "logits_p", "cotan", "output_kl_q", "lse_q_in", "lse_p_in"],
        output_names=["out_q"],
        source=source,
        ensure_row_contiguous=True,
    )
...
def _js_div_loss(logits_q, logits_p):
    ...
    outputs = _js_forward_kernel(
        ...
        output_dtypes=[dt, dt, mx.float32, mx.float32],
        template=[("T", dt), ("V", logits_q.shape[-1])],
        grid=(1024, n_outs, 1),
        threadgroup=(1024, 1, 1),
    )
    return (
        outputs[0],
        mx.stop_gradient(outputs[1]),
        mx.stop_gradient(outputs[2]),
        mx.stop_gradient(outputs[3]),
    )
...
def _js_div_loss(primals, cotangents, outputs):
    logits_q, logits_p = primals
    cotan, _, _, _ = cotangents
    _, kl_q, lse_q, lse_p = outputs
    dt = logits_q.dtype

    dp = mx.zeros_like(logits_p)
    dq = _js_backward_kernel(
        inputs=[logits_q, logits_p, cotan, kl_q, lse_q, lse_p],
        output_shapes=[logits_q.shape],
        output_dtypes=[dt],
        template=[("T", dt), ("V", logits_q.shape[-1])],
